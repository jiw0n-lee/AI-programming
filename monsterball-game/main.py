...
게임이 시작되면 여러개의 공이 무작위로 섞이고,
화면 상단에 '몬스터볼을 눌러주세요!'라는 문구가 뜨면 
1초 안에 몬스터볼을 찾아 클릭하면 성공입니다.

**치트기능 소개:
키보드의 'a'버튼이나 'q'버튼을 누르면, 몬스터볼을 클릭하지 않아도 성공이 가능합니다.

게임이 끝나고 3초가 지나면 화면이 전환되며,
Esc를 누르면 창을 닫을 수 있고, Enter를 누르면 다시 플레이가 가능합니다.

Esc를 누르면 아무때나 게임 종료 가능
